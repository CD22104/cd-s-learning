...
- 运行时多态
  ...
  - 覆盖（子类对父类覆盖）
    ...
    通过动态绑定来决定是子类还是父类，也称虚方法调用（运行时决定）
...
> 子类重载了父类方法时，运行时系统会根据调用方法的实例的类型来选择哪个方法调用，所有非final方法都会自动的进行动态绑定

#### 动态类型确定

变量instanceof类型用于判断它是不是某个变量，某个类型。
...
1. static：以声明的类型为准，与实例类型无关
2. private：子类看不见，也不会被虚化
3. final：子类不能覆盖，不存在虚化问题
